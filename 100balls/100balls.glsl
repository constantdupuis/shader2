...
void main() {
    ...
    vec2 uv = (2.*gl_FragCoord.xy-u_resolution.xy)/u_resolution.y; // pixel uv, range -1 to 1 and compensate screen aspect ratio
    vec2 uv2 = uv; 
    uv2.x *= -1.0;
    ...
    for( int i = 0; i < 100; i++)
    {
        ...
        // pick random position, same input given same output
        vec2 n2 = N22(vec2(i*i));
        // move from initial random pos
        vec2 p2  = sin(n2*t);

        float d2 = length(uv2-p2);

       // limit gradiant from 0 to 1 and add it to  m
        m2 += smoothstep(0.6, 0.001, d2);
    }
    // m range from 0 to > 1
    m = sin(m*2.0); // get gradiant back in -1, 1 range
    ...
    vec4 finalColor = colorT+colorT2;
